...
- **Backend:** see `backend/README` section below / `backend/functions/requirements.txt`.
  Requires your own Firebase service account key (not included — see Security note).
...
This repo has been scrubbed of credentials found in the original project archive
(a Firebase Admin SDK private key and a hardcoded Gemini API key). If you're
continuing this project, treat both as compromised and regenerate them — don't
reuse the originals.
