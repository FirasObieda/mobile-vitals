...
class _HistoryWidgetState extends State<HistoryWidget> {
  ...
  final positiveValues = combinedValues.where((value) => value > 0).toList();
  const requiredDays = 7;
  final hasFullWeekData =
    dataLength >= requiredDays &&
    hrSeries.length >= requiredDays &&
    sbpSeries.length >= requiredDays &&
    dbpSeries.length >= requiredDays &&
    spo2Series.length >= requiredDays &&
    hrSeries.take(requiredDays).every((value) => value > 0) &&
    sbpSeries.take(requiredDays).every((value) => value > 0) &&
    dbpSeries.take(requiredDays).every((value) => value > 0) &&
    spo2Series.take(requiredDays).every((value) => value > 0);

  final hasChartData = positiveValues.isNotEmpty && hasFullWeekData;
    ...
    double? computeAverage(List<double> values) {
    ...
    final averageHr = computeAverage(hrSeries);
    final averageSbp = computeAverage(sbpSeries);
    final averageDbp = computeAverage(dbpSeries);
    final averageSpo2 = computeAverage(spo2Series);

    final weeklySeries = hasChartData
        ? [
            FFLineChartData(
              xData: xValues,
              yData: hrSeries,
                    ...
                    child: Padding(
                      ...
                      children: [
                        ...
                        child: weeklySeries
                          ...
                          : Center(
                            child: Text(
                              'Collect seven consecutive days of vitals to view your trends.',
                              textAlign:
                                TextAlign.center,
